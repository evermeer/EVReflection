...
extension SignalProducerProtocol where Value == Moya.Response, Error == MoyaError {
    ...
    public func map<T: EVReflectable>(to type: T.Type) -> SignalProducer<T, Error> where T: NSObject {
        return producer.flatMap(.latest) { response -> SignalProducer<T, Error> in
            return SignalProducer(value: T(data: response.data))
        }
    }
    ...
    /// protocol.
    /// If the conversion fails, the signal errors.
    public func map<T: EVReflectable>(toArray type: T.Type) -> SignalProducer<[T], Error> where T: NSObject {
        return producer.flatMap(.latest) { response -> SignalProducer<[T], Error> in
            return SignalProducer(value: [T](data: response.data))
        }
    }
