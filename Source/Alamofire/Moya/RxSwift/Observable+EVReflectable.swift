...
public extension ObservableType where E == Response {
    ...
    public func Rmap<T: NSObject>(to type: T.Type, forKeyPath: String? = nil) -> Observable<T> where T: EVReflectable {
    ...
    /// If the conversion fails, the signal errors.
    public func RmapArray<T: NSObject>(to type: T.Type, forKeyPath: String? = nil) -> Observable<[T]>  where T: EVReflectable {
    return flatMap { response -> Observable<[T]> in
    ...
    public func RmapNestedArray<T: NSObject>(to type: T.Type, forKeyPath: String? = nil) -> Observable<[[T]]>  where T: EVReflectable {
        return flatMap { response -> Observable<[[T]]> in
...
public extension PrimitiveSequence where TraitType == SingleTrait, ElementType == Response {
    ...
    /// If the conversion fails, the signal errors.
    public func Rmap<T: NSObject>(to type: T.Type, forKeyPath: String? = nil) -> Single<T> where T: EVReflectable {
    ...
    /// If the conversion fails, the signal errors.
    public func RmapArray<T: NSObject>(to type: T.Type, forKeyPath: String? = nil) -> Single<[T]> where T: EVReflectable {
        return observeOn(ConcurrentDispatchQueueScheduler(qos: .background))
            .flatMap { response -> Single<[T]> in
    ...
    /// which implement the EVReflectable protocol and returns the result back on the MainScheduler
    /// If the conversion fails, the signal errors.
    public func RmapNestedArray<T: NSObject>(to type: T.Type, forKeyPath: String? = nil) -> Single<[[T]]> where T: EVReflectable {
        return observeOn(ConcurrentDispatchQueueScheduler(qos: .background))
            .flatMap { response -> Single<[[T]]> in
